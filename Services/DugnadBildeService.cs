...
public class DugnadBildeService
{
    ...
    public DugnadBildeService(IWebHostEnvironment environment)
    {
        _mappe = Path.Combine(environment.ContentRootPath, "dugnadsbilder");

        Directory.CreateDirectory(_mappe);
    }

    public async Task<List<DugnadBilde>> LagreBilderAsync(
        int dugnadId,
        IEnumerable<IFormFile>? filer)
    {
        ...
        var resultat = new List<DugnadBilde>();
    ...
    public void SlettBilde(DugnadBilde bilde)
    {
